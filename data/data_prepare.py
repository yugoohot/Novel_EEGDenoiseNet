...
def prepare_data(EEG_all, noise_all, combin_num, train_per, noise_type):
    ...
    #生成了行列数相等的EEG_train和NOISE_train
    ...
    for i in range (EEG_train.shape[0]):
        ...
        coe=get_rms(eeg)/(get_rms(noise)*SNR_train[i]) #均方根，是干什么的
        noise = noise*coe
        neeg = noise+eeg
    ...
    for i in range(noiseEEG_train.shape[0]):
        ...
        eeg_train_all_std = EEG_train[i] / np.std(noiseEEG_train[i])  #纯净的EEG片段/污染EEG的标准差
        EEG_train_end_standard.append(eeg_train_all_std)

        noiseeeg_train_end_standard = noiseEEG_train[i] / np.std(noiseEEG_train[i])   #污染EEG片段/污染EEG的标准差
    ...
    for i in range(noise_EEG_test.shape[0]):
        ...
        # store std value to restore EEG signal
        std_value = np.std(noise_EEG_test[i]) #输出了被污染的EEG的测试集的标准差
        std_VALUE.append(std_value)
